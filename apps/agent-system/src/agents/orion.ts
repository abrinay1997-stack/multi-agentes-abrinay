...
const DEFAULT_QUERIES_ORION = [
  'tendencias YouTube música urbana latina artista emergente 2026',
  'YouTube Shorts rap reggaeton colombia panama estrategia crecimiento',
]

export class OrionAgent extends BaseAgent {
  readonly name: AgentName = 'orion'
  readonly outputType = 'tendencias'

  protected async execute(ctx: RunContext): Promise<unknown> {
    this.thinking('Analizando tendencias YouTube...')
    ...
    const queries = cfg.tavilyQueries ?? DEFAULT_QUERIES_ORION
...
DATOS DE ABRINAY EN YOUTUBE:
${scoutData?.youtube_data ?? 'No disponible'}

TENDENCIAS ENCONTRADAS (búsqueda web):
${trendsRaw}

Selecciona las 3 tendencias de YouTube más relevantes para la campaña Licencia P de Abrinay.
