...
const DEFAULT_QUERIES_KIRA = [
  'tendencias TikTok dembow trap urbano latino 2026',
  'TikTok viral música urbana colombia panama artista emergente',
]

export class KiraAgent extends BaseAgent {
  readonly name: AgentName = 'kira'
  readonly outputType = 'tendencias'

  protected async execute(ctx: RunContext): Promise<unknown> {
    this.thinking('Analizando tendencias TikTok...')
    ...
    const queries = cfg.tavilyQueries ?? DEFAULT_QUERIES_KIRA
...
DATOS DE ABRINAY EN TIKTOK:
${scoutData?.tiktok_data ?? 'No disponible'}

TENDENCIAS ENCONTRADAS (búsqueda web):
${trendsRaw}

Selecciona las 3 tendencias de TikTok más relevantes para la campaña Licencia P de Abrinay.
